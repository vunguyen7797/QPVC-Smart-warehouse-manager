...
import 'package:qpv_face_scanner/cognitiveFaceServices/common/request_method.dart';
import 'package:qpv_face_scanner/cognitiveFaceServices/contract/identify_result.dart';
import 'package:qpv_face_scanner/cognitiveFaceServices/rest/web_service_request.dart';

import 'contract/face.dart';
import 'contract/verify_result.dart';
...
//    }
  ...
  Future<IdentifyResult> identityInLargePersonGroup(
      {String largePersonGroupId,
      List faceIds,
      double confidenceThreshold = 0.5,
      int maxNumOfCandidatesReturned = 1}) async {
    assert(faceIds.length != 0 || largePersonGroupId != null);

    print("Identifying....");

    String uri = '$serviceHost/$IDENTIFY_QUERY';
    Map<String, Object> params = {};

    if (faceIds.isNotEmpty && largePersonGroupId != null) {
      params["faceIds"] = faceIds;
      params["largePersonGroupId"] = largePersonGroupId;
      params["maxNumOfCandidatesReturned"] = maxNumOfCandidatesReturned;
    ...
    //List<IdentifyResult> results = json.map((Map<String, dynamic>) => IdentifyResult.fromJson(json as Map<String, dynamic)).toList;

    return IdentifyResult.fromJson(json[0] as Map<String, dynamic>);
  }
}
